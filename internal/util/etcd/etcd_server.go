...
func InitEtcdServer(etcdCfg *paramtable.EtcdConfig) error {
	if etcdCfg.UseEmbedEtcd.GetAsBool() {
		var initError error
		initOnce.Do(func() {
			path := etcdCfg.ConfigPath.GetAsString()
			...
			cfg.Dir = etcdCfg.DataDir.GetAsString()
			cfg.LogOutputs = []string{etcdCfg.EtcdLogPath.GetAsString()}
			cfg.LogLevel = etcdCfg.EtcdLogLevel.GetAsString()
			...
			etcdServer = e
			log.Info("finish init Etcd config", zap.String("path", path), zap.String("data", etcdCfg.DataDir.GetAsString()))
		})
		return initError
	}
	return nil
