...
func (suite *HTTPServerTestSuite) TestHealthzHandler() {
	...
	healthz.Register(&MockIndicator{"m1", internalpb.StateCode_Healthy})
	...
	healthz.Register(&MockIndicator{"m2", internalpb.StateCode_Abnormal})
	req, _ = http.NewRequest(http.MethodGet, url, nil)
	req.Header.Set("Content-Type", "application/json")
	resp, err = client.Do(req)
	suite.Nil(err)
...
func (m *MockIndicator) Health(ctx context.Context) internalpb.StateCode {
	return m.code
}
