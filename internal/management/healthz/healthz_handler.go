...
type Indicator interface {
	GetName() string
	Health(ctx context.Context) internalpb.StateCode
}

type IndicatorState struct {
	Name string               `json:"name"`
	Code internalpb.StateCode `json:"code"`
}
...
var defaultHandler *HealthHandler = &HealthHandler{}
...
func writeJSON(w http.ResponseWriter, r *http.Request, resp *HealthResponse) {
	...
	if err != nil {
		log.Warn("faild to send respone", zap.Error(err))
	}
	w.Write(bs)
}
